...
trait DSL {
  ...
  def zombieInvasion(
    ...
    walkNoise : Double = physic.walkNoise,
    runNoise : Double = physic.runNoise,
    exhaustNoise : Double = physic.exhaustNoise,
    fightNoise : Double = physic.fightNoise,
    ...
    val generatedAgents =
      agents.flatMap { a =>
        AgentGenerator.generateAgent(
          ...
          walkNoise = walkNoise,
          runNoise = runNoise,
          exhaustNoise = exhaustNoise,
          fightNoise = fightNoise
        )
      }
    ...
    def generateAgent(
      ...
      def generateHuman(human: Human) = {
        ...
        zombies.agent.Human(
          ...
          rng = random,
          noise = 0.0,
          walkNoise = human.walkNoise.getOrElse(walkNoise),
          runNoise = human.runNoise.getOrElse(runNoise),
          exhaustNoise = human.exhaustNoise.getOrElse(exhaustNoise),
          fightNoise = human.fightNoise.getOrElse(fightNoise))
      }
      ...
      def generateZombie(zombie: Zombie) = {
        zombies.agent.Zombie(
          ...
          random = random,
          noise = 0.0,
          walkNoise = zombie.walkNoise.getOrElse(walkNoise),
          runNoise = zombie.runNoise.getOrElse(runNoise)
        )
      }
  ...
  case class Human(
    ...
    informed:  AgentGenerator.Optional[Boolean] = None,
    walkNoise: AgentGenerator.Optional[Double] = None,
    runNoise: AgentGenerator.Optional[Double] = None,
    exhaustNoise: AgentGenerator.Optional[Double] = None,
    fightNoise: AgentGenerator.Optional[Double] = None) extends AgentGenerator
  ...
  case class Zombie(
    ...
    location:  AgentGenerator.Optional[Location] = None,
    walkNoise: AgentGenerator.Optional[Double] = None,
    runNoise: AgentGenerator.Optional[Double] = None) extends AgentGenerator
