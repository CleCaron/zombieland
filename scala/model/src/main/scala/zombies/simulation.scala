...
object simulation {
  ...
  def step(step: Int, simulation: Simulation, neighborhoodCache: NeighborhoodCache, rng: Random) = {
    ...
    val (na1, moveEvents) =
      ai.map { a0 =>
        ...
        val noisiest = a0 match {
          case z: Zombie if(ns.isEmpty) => ns
          case z: Zombie => Array(ns.max(Ordering.by((a:Agent) => { a match {
            case z2: Zombie => z2.noise / space.distance(Agent.position(z), Agent.position(z2))
            case h: Human => h.noise / space.distance(Agent.position(z), Agent.position(h))
          }})))
          case h: Human => ns
        }

        val evolve =
          Agent.inform(noisiest, w1, rng) _ andThen
          Agent.alert(noisiest, rng) _ andThen
          Agent.takeAntidote _ andThen
          Agent.chooseRescue _ andThen
          Agent.run(noisiest, rng) _ andThen
          Agent.metabolism(rng) _ andThen
          Agent.generateNoise(rng) _

        val a1 = evolve(a0)

        val (a2, ev) = Agent.changeDirection(w1, simulation.rotationGranularity, noisiest, rng)(a1)
 ...
 type SimulationResult = (List[Simulation], List[Vector[Event]])
  ...
  object physic {
    ...
    val walkNoise = 1.0
    val runNoise = 10.0
    val exhaustNoise = 5.0
    val fightNoise = 100.0
  }
