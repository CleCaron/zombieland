...
object agent {
  ...
  sealed trait Agent
  case class Human(position: Position, velocity: Velocity, metabolism: Metabolism, perception: Double, maxRotation: Double, followRunningProbability: Double, fight: Fight, rescue: Rescue, canLeave: Boolean, antidote: AntidoteMechanism, function: Human.Function, noise: Double, walkNoise: Double, runNoise: Double, exhaustNoise: Double, fightNoise: Double) extends Agent
  case class Zombie(position: Position, velocity: Velocity, walkSpeed: Double, runSpeed: Double, perception: Double, maxRotation: Double, pursuing: Boolean, canLeave: Boolean, noise: Double, walkNoise : Double, runNoise: Double) extends Agent
  ...
  object Agent {
        ...
        for {
          ...
          case c: Floor if c.humanEntranceLambda.isDefined =>
            ...
            def human = Human.apply(
              ...
              rng = random,
              noise = 0.0,
              walkNoise = simulation.walkNoise,
              runNoise = simulation.runNoise,
              exhaustNoise = simulation.exhaustNoise,
              fightNoise = simulation.fightNoise).copy(position = World.cellCenter(world, (x, y)))
    ...
    def generateNoise(rng : Random)(a: Agent) =
      a match {
        case h : Human if h.metabolism.run => h.copy(noise = h.runNoise)
        case h : Human if h.metabolism.exhausted => h.copy(noise = h.exhaustNoise)
        case h : Human if h.rescue.informed && rng.nextDouble < 0.9 => h.copy(noise = 0.0)
        case h : Human => h.copy(noise = h.walkNoise)
        case z : Zombie if z.pursuing => z.copy(noise = z.runNoise / 5)
        case z : Zombie => z.copy(noise = z.walkNoise / 5)
      }
    ...
    def run(neighbors: Array[Agent], rng : Random)(a: Agent) =
      a match {
        case h: Human if neighbors.exists(Agent.isZombie) && h.rescue.informed && (rng.nextDouble() <= 0.8) => h
        case h: Human if neighbors.exists(Agent.isZombie) || h.rescue.reach => Human.run(h)
        case z: Zombie if neighbors.length > 0 => neighbors(0) match {
          case h : Human if h.noise > 0.0 => Zombie.pursue(z)
          case z2 : Zombie if z2.noise > 0.0 => Zombie.pursue(z)
          case _ => Zombie.stopPursuit(z)
        }
    ...
    def changeDirection(world: World, granularity: Int, neighbors: Array[Agent], rng: Random)(agent: Agent) = {
      ...
      def pursue(z: Zombie, nh: Array[Agent], rng: Random) = {
      ...
      agent match {
        ...
        case z: Zombie =>
          getTrapped(z, rng) match {
            ...
            case None =>
              neighbors match {
                case nh if ! nh.isEmpty => (pursue (z, nh, rng), Some (PursueHuman (z) ) )
  ...
  object Human {
    def apply(
      ...
      Human(p, v, Metabolism(walkSpeed, runSpeed, exhaustionProbability, false, false), perception, maxRotation, followRunningProbability, fight, rescue = rescue, canLeave = canLeave, antidote = antidote, function = function, noise = 0.0, walkNoise, runNoise, exhaustNoise, fightNoise)
    }
  ...
  object Zombie {
    def apply(
      ...
      Zombie(p, v, walkSpeed, runSpeed, perception, maxRotation, pursuing = false, canLeave = canLeave, noise = 0.0, walkNoise, runNoise)
    }
